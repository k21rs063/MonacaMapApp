describe('MonacaMapApp', function () {
    ...
    it('Handling event map - Click full screen, move map', function () {
        // Test click full screen map
        cy.get('button[class="gm-control-active gm-fullscreen-control"]').should('be.visible')
        cy.get('button[class="gm-control-active gm-fullscreen-control"]').click()
    ...
    it('Handling show all location (全件) - Validate Alert Text and Click OK', () => {
        ...
        cy.get('#all').click()
        cy.get('div[aria-label="<h2>my location</h2><p>my location</p>"]').should('be.visible')
    })
